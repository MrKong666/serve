...
/*TCP 是面向字节流的，数据包可能会在传输中被拆分或合并。通过两个阶段解决了这个问题：
固定头部读取：AsyncReadHead 总是读取 HEAD_TOTAL_LEN（4字节）。
动态长度解析：从头部解析出 msg_len 后，再调用 AsyncReadBody 读取指定长度的数据负载。
健壮性检查：你在解析长度后检查了 msg_len > MAX_LENGTH。防止恶意攻击者发送伪造的超大长度字段导致服务器内存耗尽。*/